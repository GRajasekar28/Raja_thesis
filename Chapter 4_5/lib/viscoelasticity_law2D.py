...
## minimization of F = integral of f
## f = g1(d) \psi_0  + g2(d) \phi_v + Y_c h(d)
## \psi_0 - undamged free energy
## \phi_v - undamaged viscous dissipation potential
## h(d) - damage softening function
...
def HCubic(c):      return HPoly(coef=[0,2,c, 5],name='h(d) = 2d+'+str(c)+'d²+5d^3') 
...
def HCohesive(lm,a=1) : return HRPoly([a*i for i in[0.,2.,-1.]],[1.,-2,(1.+2*lm),-2.*lm, lm**2], name = 'cohesive l='+str(lm))
...
def Gpow3(): return HPoly(coef=[1.,-3.,3.,-1],name= 'g(d) = (1-d)^3')
def Gpow4(): return HPoly(coef=[1.,-4.,6.,-4,1],name= 'g(d) = (1-d)^4')
def Gpow5():   return HPoly(coef=[1.,-5.,10.,-10,5,-1],name= 'g(d) = (1-d)^5')
def Gpow6():   return HPoly(coef=[1.,-6.,15.,-20,15,-6,1],name= 'g(d) = (1-d)^6')
...
class G_linear_approx:
    def __init__(self, a=.2, name = 'G_linear_approx'):
        self.name = name
        self.a = a
    def __call__(self,d):
        #print(min(d),max(d))
