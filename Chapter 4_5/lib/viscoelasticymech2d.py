...
"""IMP: changes in code requried for including convergene critrion based on  res_eps_i_inf and res_stress_balance.. 
not inlcuding this causes issues in certain cases """
...
class Mechanics2D:
    ...
    def energy(self,u = None, eps_i=None, d= None, eps_i_n=None,DT=None) :
        ## eps1 - (n)  current time step 
         ...
         if DT is None: raise
         strain = self.strain(u) 
         phi = self.law.potential(strain, eps_i, d.squeeze(), eps_i_n, DT)
    ...
    def solveDisplacementFixedDLinear(self, u0, eps_i_n, DT, d=None,imposed_displacements = None, imposed_nodal_forces = None, 
                                      solveroptions = {'linsolve':'cholmod'}, linearizedrigidbodyconstraints = None):
    ...
    def solveDGlobal(self, dmin, dprec, u, eps_i_np1, eps_i_n=None,DT=None, solveroptions={'lipmeasure':'triangle','snapthreshold':0.999}, 
        ...
        resopt = self.lipprojector.lipProjMinConvex(F, lipconstrain, kktsolveroption = 'umfpack')
    ...
    def alternedSolver(self, dmin, dguess, DT,un=None, eps_i_n = None, 
        ...
        while(it <= max_iter+1 and (not converged)):
            ...
            resu  = solverdisp(self, u0 = u, eps_i_n = eps_i_n, DT = DT, d = d,  
            ...
            eu = self.energy(u,eps_i_np1,d, eps_i_n, DT)  
            ...
            if damage_calc:
                ...
                delta_d = 0;
                d_nodes = None
            ed = self.energy(u,eps_i_np1,d, eps_i_n, DT)
