...
## load the mesh
basemeshname = 'scb_symmetric_r23'    ## name of the mesh
...
simulation_id = str(9)+'_fit_2707_lm_czm_change_yc_gd6_new'     ## give new id for performing simulation on same mesh with different setting (to save in a different folder)
...
if not unilateral:
    ...
    g1 = g
    H = visclaw.HCohesive(lm)   ## softening function   h(d)
    law = visclaw.viscoElasticity2dPlaneStrain(ni,lamb,mu,tau,g1=g,g2=g,Yc = Yc,Gc = Gc,h=H,var_con=False)
else:
    ...
    H = visclaw.HCohesive(lm, 6/2)     ## h(d)
    #H = visclaw.HQuadratic()
    #H = visclaw.HCubic(5)
    #H = visclaw.H_TLS_1()
    law = visclaw.viscoElasticity2dPlaneStrain_ASSIM(ni,lamb,mu,tau,split_choice=split_choice,Yc  = Yc,Gc = Gc,
                                                     g1 =g1,g2=g2,h=H)
...
videl_d0 = []
a_low = 0/1000     ;   a_left = 65/1000
a_uppe = 71e-3     ;   a_right = np.inf
...
if real_time_plot:
    ...
    ax0.set_ylabel(r'Reaction Force (N/mm)')
...
while u_appl < max_uy:
    ...
    if res['ad_tim']:
        ...
        if ad_tim_count > 10:  # max count for ad_time_stepping
