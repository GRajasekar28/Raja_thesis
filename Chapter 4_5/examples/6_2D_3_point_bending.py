...
#######################################################################
#########  EXAMPLE 6: 3 POINT BENDING (SENB test) (Mode -1 fracture) ###########################

### ADAPTIVE TIME STEPPING USED TO ENSURE CONVERGENCE OF SOLUTION

## Geometry from  http://dx.doi.org/10.1016/j.engfracmech.2010.06.023
#######################################################################
...
## load the mesh
basemeshname = 'beam_3_point_bending_r2'     ## name of the mesh
meshid = ''
...
simulation_id = str(1)+'_speed_p1mmps_gd2_dt_p5_stopping_criteria'     ## give new id for performing simulation on same mesh with different setting (to save in a different folder)
expname = basemeshname+ simulation_id

### for outputfiles 
respath = '../tmp/3_point_bending/280723'
...
resume_path = r'D:\VBox shared folder\visco v1.7\tmp\3_point_bending\280723\beam_3_point_bending_r21_speed_p01mmps_gd2_dt_5_r2_energy_check/results/results_u_0.00755173339843753_.npz'
resume_path2 = r'D:\VBox shared folder\visco v1.7\tmp\3_point_bending\280723\beam_3_point_bending_r21_speed_p01mmps_gd2_dt_5_r2_energy_check/force_disp.npz'
resume_disp = 0.00755173339843753   ## can extract from the file name of resume_path
ind_resume = 315  ## time step indices corresponding to resumign simulation
"""
...
Yc = 750    ## critical energy release rate (J/m^3)
lc = 5e-3  ## characterstic/regularization length (m)
...
adap_time_step = 0        ### set to True for adaptive time stepping to ensure proper cpnvergence (might take longer time)
max_iter_adap_time_step = 5  ##integer (number of adaptions allowed)
...
"""
videl_d0 = []
a_low = 0/1000     ;   a_left = 110/1000
a_uppe = 38/1000     ;   a_right = 150/1000
...
DT1 = DT; 
adap_time_step_tmp = adap_time_step
while u_appl < max_uy:
    u_appl += speed*(DT1)
    ...
    res= alternedsolver( dmin =dmin, dguess =d.copy(), DT=DT1, un= u.copy(), eps_i_n = eps_i.copy(), 
                ...
                damage_calc=damage_calc, imposed_d_0=videl_d0,adap_time_step=adap_time_step_tmp)
    
    
       
        
    if adap_time_step_tmp and res['ad_tim']:
        ...
        np.savez(respath1+'/force_disp.npz',Fx = np.array(Fx).squeeze(), Fy = np.array(Fy).squeeze(),
                 ...
                 wi = np.array(energ['wi']),cl = np.array(energ['cl']), timeu = np.array(timeu),timed = np.array(timed))
...
logger.info('Time spend on equilibrium solver :' + str(sum(timeu)) + 's')
logger.info('Time spend on damage solver      :' + str(sum(timed)) + 's')
